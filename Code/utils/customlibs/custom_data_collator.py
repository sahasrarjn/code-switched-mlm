...
class DataCollatorForLanguageModeling(DataCollator):
    ...
    def collate_batch(self, examples) -> Dict[str, torch.Tensor]:
        ...
        if self.mlm:
            inputs, labels = self.mask_tokens([batch,tomask, texts])
    ...
    def mask_tokens(self, inputs) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        with open('act_mask.txt', 'a+') as f:
            for i in range(len(texts)):
                f.write(texts[i]+'\n')
                s1 = ""
                for j in masked_indices[i]:
                    if j:
                        s1+="MASK"
                    else:
                        s1+="NOMASK"
