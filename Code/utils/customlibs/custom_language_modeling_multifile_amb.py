...
def getIdFromMask(mask):
	if mask == "NOMASK": return -1
	elif mask == "MASK0": return 0
	elif mask == "MASK1": return 1
	elif mask == "MASK2": return 2
	else: raise Exception("Invalid mask type")

class LineByLineTextDataset_amb(Dataset):
	"""
	This will be superseded by a framework-agnostic approach
	soon.
	"""
	...
	def __init__(self, tokenizer: PreTrainedTokenizer, files: list, block_size: int):
		...
		for i, file_path in enumerate(files):
			...
			tomasks = [[getIdFromMask(msk) for msk in t] for t in tomasks]
	...
	def __getitem__(self, i) -> torch.Tensor:
		return torch.tensor(self.examples[i], dtype=torch.long), torch.tensor(self.tomasks[i], dtype=torch.int), self.classes[i]
