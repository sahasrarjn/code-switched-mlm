...
class LineByLineTextDataset(Dataset):
	...
	def __init__(self, tokenizer: PreTrainedTokenizer, file_path: str, block_size: int):
		...
		for j in range(len(self.examples)):
			if len(self.examples[j]) != len(tomasks[j]):
				...
				break
		for j in range(len(self.examples)): assert len(self.examples[j]) == len(tomasks[j]),  pdb.set_trace() #"Tokenized sentence and token-mask indicators are of different lengths, {} and {} for {}".format(len(self.examples[j]), len(tomasks[j]), lines[j])
	...
	def __getitem__(self, i):
		return torch.tensor(self.examples[i], dtype=torch.long), torch.tensor(self.tomasks[i], dtype=torch.bool), self.texts[i]
