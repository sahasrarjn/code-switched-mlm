...
from customlibs.residual_bert import ResAuxBertForMaskedLM

# For all MLM techniques
from customlibs.custom_data_collator_multifile import DataCollatorForLanguageProbeModeling
from customlibs.custom_language_modeling_probing import LineByLineTextProbeDataset
...
def get_dataset(
    ...
    if args.line_by_line:
        return LineByLineTextProbeDataset(tokenizer=tokenizer, files=files, block_size=args.block_size)
...
def main():
    ...
    model = ResAuxBertForMaskedLM.from_pretrained(
        model_args.model_name_or_path,
    ...
    # A data collator which handles both the MLM and probing data 
    data_collator = DataCollatorForLanguageProbeModeling(
        tokenizer=tokenizer,
        mlm=data_args.mlm,
