...
def train(args, train_dataset, valid_dataset, model, tokenizer, labels):
    ...
    for _ in train_iterator:
        ...
        # Recording test f1 scores
        results = evaluate(args, model, tokenizer, labels, 'test')
        ...
        file_ = open("./TSTG/random/logs_42_test.txt.orig", "a+")  # append mode
        file_.write(str(results['f1']))
        file_.write("\n")
        file_.close()
        ...
        if results.get('f1') > best_f1_score and args.save_steps > 0:
            ...
            tokenizer.save_pretrained(args.output_dir)
            torch.save(args, os.path.join(args.output_dir, "training_args.bin"))
        file_ = open("./TSTG/random/logs_42_valid.txt.orig", "a+")  # append mode
        file_.write(str(results['f1']))
        file_.write("\n")
        file_.close()
