...
python3.6 $PREPROCESS_DIR/preprocess_drqa.py --data_dir $ORIGINAL_DATA_DIR
...
pip install spacy
python3.6 -m spacy download xx_ent_wiki_sm
python3.6 -c "import nltk;nltk.download(['punkt', 'averaged_perceptron_tagger', 'maxent_ne_chunker', 'words'])"
./download.sh
...
>     random.seed(0)
EOF
python3.6 scripts/distant/generate.py $PART1 $PART2 $PREPROCESS_DIR --tokenizer spacy --dev-split 0.2 --n-docs 1 --workers 1

cd ./..
# Squad format processor
python3.6 $PREPROCESS_DIR/preprocess_qa_en_hi.py --output_dir $PROCESSED_DIR